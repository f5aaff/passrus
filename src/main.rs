...
**/
fn pass_2_key(input: &str, mut salt: [u8; 32]) -> Result<([u8; 32], [u8; 32]), argon2::Error> {
    if salt.is_empty() {
...
fn main() {
    ...
    let key_n_salt = match pass_2_key(pass, [0u8; 32]){
        Ok(res) => {
            res
        },
        Err(error) => {
    ...
    let _enc_res = match encrypt_file_mem_with_salt(filepath, "w_salt_enc", &key, &nonce, &salt) {
        Ok(res) => {
    ...
    let _dec_res = match decrypt_file_mem_gen_key("w_salt_enc", "w_salt_dec", pass) {
        Ok(res) => {
